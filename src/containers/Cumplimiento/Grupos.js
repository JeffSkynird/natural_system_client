...
import { obtenerTodosGrupos, eliminarGrupo } from '../../utils/API/grupos.js'
...
export default function Grupos(props) {
    ...
    React.useEffect(() => {
        ...
        if (initializer.usuario != null) {
            obtenerTodosGrupos(setData, initializer);
        }
    ...
    const eliminar = () => {
        eliminarGrupo(open.id, initializer, cargarData)

    }
    const cambiar = (text) => {
        let temp = data.backup.filter(word =>removeAccent(word.name).toLowerCase().includes(removeAccent(text).toLowerCase()));
        ...
        <div style={{ paddingTop: 15, paddingLeft: 15, paddingRight: 15 }}>
            ...
            <Grid container justify='space-between' alignItems="center" style={{ marginBottom: '10px' }}>
                ...
                <Box>
                    ...
                    <Button startIcon={<AddCircleOutline />} variant="contained" color="secondary" onClick={() => props.history.push('/grupos/crear')}>
                        Crear
                    </Button>
            ...
            <Grid container>
                <Grid item xs="12">
                    <MaterialTable
                        ...
                        data={
                            data.data.map((e) => {
                                return ({ ...e, names: e.supervisor.nombres })
                            })
                        }
                        ...
                        actions={[
                            {
                                ...
                                onClick: (event, rowData) => props.history.push("/grupos/editar", rowData)
                            },
                            {
                                icon: TableIcons.AssignmentIndIcon,
                                tooltip: "Asignar vendedores",

                                onClick: (event, rowData) => props.history.push("/grupos/asignar", rowData)
                            },
                            {
