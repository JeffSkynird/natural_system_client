...
import { obtenerManagers, eliminarManager } from '../../utils/API/managers.js'
...
export default function Clientes(props) {
    ...
    React.useEffect(() => {
        ...
        if (initializer.usuario != null) {
            obtenerManagers(setData, initializer);
        }
    ...
    const eliminar = () => {
        eliminarManager(open.id, initializer, cargarData)

    }
    const cambiar = (text) => {
        let temp = data.backup.filter(word => {
                return removeAccent(word.full_name).toLowerCase().includes(removeAccent(text).toLowerCase()) ||
                    word.dni.includes(text,0) ||
                    word.email.includes(text,0)
            });
        ...
        <div style={{ paddingTop: 15, paddingLeft: 15, paddingRight: 15 }}>
            ...
            <Grid container justify='space-between' alignItems="center" style={{ marginBottom: '10px' }}>
                ...
                <Box>
                    ...
                    <Button startIcon={<AddCircleOutline />} variant="contained" color="secondary" onClick={() => props.history.push('/managers/crear')}>
                        Crear
                    </Button>
            ...
            <Grid container>
                <Grid item xs="12">
                    <MaterialTable
                        ...
                        columns={[
                            { title: "Cédula", field: "dni" },
                            { title: "Nombres", field: "full_name" },
                            { title: "Correo", field: "email" },
                            { title: "Celular", field: "cellphone" },
                            { title: "Teléfono convencional", field: "landline" },
                        ...
                        actions={[
                            {
                                ...
                                onClick: (event, rowData) => props.history.push("/managers/editar", rowData)
                            },
